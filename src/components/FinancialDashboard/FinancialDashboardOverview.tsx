                    ...
                    cards.map((card, idx) => (
                        <div key={idx} className={`${card.bg} p-5 md:p-7 rounded-[2.5rem] border border-[#E9EFF5] shadow-sm hover:shadow-md transition-all group relative overflow-hidden active:scale-[0.98]`}>
                            ...
                            <div className="flex justify-between items-start mb-5 relative z-10">
                                <div></div>
                                <div className={`flex items-center gap-1 text-[10px] md:text-xs font-bold px-2.5 py-1.5 rounded-full ${card.isUp ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'
                                    }`}>
                                    {card.isUp ? (
                ...
                <div className="lg:col-span-2 bg-white p-6 md:p-10 rounded-[2.5rem] border border-[#E9EFF5] shadow-sm relative overflow-hidden group">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 relative z-10">
                        <div>
                            ...
                            <p className="text-xs md:text-sm text-text-muted font-bold">Performa keuangan 7 hari terakhir</p>
                        </div>
                        <div className="flex bg-bg-secondary p-1 rounded-xl border border-[#E9EFF5] w-full sm:w-auto">
                            ...
                            <button className="flex-1 sm:flex-none px-4 py-2 text-xs font-bold text-text-muted hover:text-black">Bulanan</button>
                        </div>
                    </div>
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={trendData}>
                                ...
                                <XAxis
                                    ...
                                    axisLine={false}
                                    tickLine={false}
                                    tick={{ fontSize: 10, fontWeight: 'bold', fill: '#666666' }}
                                    dy={10}
                                />
                                <YAxis hide />
                                <Tooltip
                    ...
                    <div className="bg-white p-8 rounded-[2.5rem] border border-[#E9EFF5] shadow-sm relative overflow-hidden group active:scale-[0.98] transition-transform">
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="flex items-center gap-2">
                                <span className="flex items-center gap-1 bg-emerald-50 text-emerald-600 px-2 py-1 rounded-lg text-[10px] font-bold underline">
                                ...
                                <span className="text-[10px] font-bold text-text-muted tracking-wide uppercase">Naik dari Feb</span>
                            </div>
                        </div>
                    </div>
